...
import { GetMonthRevenueUseCase } from './get-month-revenue.ts'

let inMemoryProvidedServicesRepository: InMemoryProvidedServicesRepository
let inMemoryServicesRepository: InMemoryServicesRepository
let inMemoryUsersRepository: InMemoryUsersRepository

let sut: GetMonthRevenueUseCase

describe('Get Month Revenue', () => {
  beforeEach(() => {
    ...
    sut = new GetMonthRevenueUseCase(inMemoryProvidedServicesRepository)
  })
  ...
  it('should be possible to get current month revenue', async () => {
    ...
    expect(result.value?.revenue).toBe(4500 * 20)
